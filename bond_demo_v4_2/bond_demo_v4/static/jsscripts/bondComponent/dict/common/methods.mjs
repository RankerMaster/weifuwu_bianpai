...
function autoArrange (graphStruct, h, w) {
  ...
  let edgeStock = nodes_pure.map((_, k) => [k]);
  let edges_ = [];
...
function exportStruct (graphStruct) {
  ...
  graphStruct.nodes.forEach(itm => {
    let node = {
      ...
      outputType: itm.outputType,
      inCount: itm.inCount,
      outCount: itm.outCount,
      showConfig: {
  ...
  graphStruct.edges.forEach(itm => {
    linkList.push({
      id: itm.id,
      from: itm.sourceNodeId,
      to: itm.targetNodeId,
      fromName: idMap[itm.sourceNodeId],
      toName: idMap[itm.targetNodeId],
      type: itm.type,
      showConfig: {
  ...
  return {
    nodes: nodeList,
    edges: linkList.sort((itmA, itmB)=>{if(itmA.id>itmB.id){return 1}else if(itmA.id!=itmB.id){return -1} else{return 0}})
  }
}
...
function importStruct (graphStruct, showType='edit', h=900, w=1200) {
  ...
  graphStruct.edges.forEach(itm => {
    ...
    let textx0 = itm.showConfig.startAnchor.x;
    let textx1 = itm.showConfig.endAnchor.x;
    if (itm.showConfig.interAnchors && itm.showConfig.interAnchors.length > 1){
      textx1 = itm.showConfig.interAnchors[1].x;
    }
    let texty0 = itm.showConfig.startAnchor.y;
    let texty1 = itm.showConfig.endAnchor.y;
    if (itm.showConfig.interAnchors && itm.showConfig.interAnchors.length > 1){
      texty1 = itm.showConfig.interAnchors[1].y;
    }
    linkList.push({
